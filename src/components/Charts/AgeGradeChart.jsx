            ...
            {payload.map((entry, index) => (
              <div key={index} className="flex items-center justify-between min-w-[120px]">
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-gray-600">Age {entry.name}:</span>
                </div>
  ...
  return (
    <div className="w-full relative">
      ...
      <div className="absolute top-4 left-6 z-10">
        <h3 className="text-lg font-semibold text-gray-800 bg-white/90 px-2 py-1 rounded">Age Distribution by Grade</h3>
      </div>
      ...
      <ResponsiveContainer width="100%" height={400}>
        ...
        >
          ...
          />
          {ages.map((age) => (
            <Bar key={age} dataKey={age} fill={ageColors[age] || ageColors.default} name={age} radius={[2, 2, 0, 0]} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    </div>
